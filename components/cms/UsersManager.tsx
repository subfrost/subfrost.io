...
import { createUser, setUserRole, setUserActive, resetPassword } from "@/actions/cms/users"
...
export function UsersManager({ users, meId }: { users: UserRow[]; meId: string }) {
  ...
  function onCreate(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (res.ok) { setEmail(""); setName(""); setPassword(""); setRole("AUTHOR"); refresh() }
  ...
  return (
    <div className="space-y-8">
      <form onSubmit={onCreate} className="grid gap-4 rounded-xl border border-zinc-800 bg-zinc-900/40 p-5 sm:grid-cols-2">
        ...
        <div className="space-y-1.5"><Label className="text-zinc-300">Temporary password</Label><Input type="text" value={password} onChange={(e) => setPassword(e.target.value)} required className={inputCls} /></div>
            ...
            {users.map((u) => {
              ...
              return (
                <tr key={u.id} className="border-t border-zinc-800">
                  ...
                  <td className="px-4 py-3">
                    <select value={u.role} disabled={isMe || pending}
                      onChange={(e) => startTransition(async () => { await setUserRole(u.id, e.target.value as Role); refresh() })}
                  ...
                  <td className="px-4 py-3"><span className={u.active ? "text-emerald-300" : "text-zinc-500"}>{u.active ? "Active" : "Disabled"}</span></td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-2">
                      <Button size="sm" variant="ghost" disabled={isMe || pending}
                        onClick={() => startTransition(async () => { await setUserActive(u.id, !u.active); refresh() })}>
                        {u.active ? "Disable" : "Enable"}
                      </Button>
                      <Button size="sm" variant="ghost" disabled={pending}
                        onClick={() => { const np = prompt(`New password for ${u.email} (min 8):`); if (np) startTransition(async () => { const r = await resetPassword(u.id, np); if (!r.ok) alert(r.error) }) }}>
                        Reset password
                      </Button>
                    </div>
                  </td>
