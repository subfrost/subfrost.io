        ...
        return (
          <div key={group.key}>
            <button
              ...
              className="flex w-full items-center gap-2 rounded-[6px] px-2 py-2 text-[#93a6bb] outline-none transition-colors hover:text-[#e9f0f7] focus-visible:ring-2 focus-visible:ring-[#a7c6dc]"
            >
              <GroupIcon size={16} />
              <span className="font-medium">{group.label}</span>
              <span className="ml-auto flex items-center gap-1.5">
                {!open && hasActive && (
                  <span className="h-1.5 w-1.5 rounded-full bg-[#a7c6dc]" aria-hidden />
                )}
                <ChevronRight
                  size={14}
                  className={`text-[#667b92] transition-transform ${open ? "rotate-90" : ""}`}
                />
              </span>
            </button>
            {open && (
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-2 rounded-[6px] px-2 py-2 outline-none transition-colors focus-visible:ring-2 focus-visible:ring-[#a7c6dc] ${
                        active
                          ? "bg-white/[0.07] text-[#e9f0f7]"
                          : "text-[#93a6bb] hover:text-[#e9f0f7]"
                      }`}
                    >
                      <ItemIcon size={15} />
                      {item.label}
