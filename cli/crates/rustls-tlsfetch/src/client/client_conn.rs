...
pub struct ClientConfig {
    ...
    /// tlsfetch fork extension: an opt-in
    /// [`ClientHelloMutator`][crate::client::ClientHelloMutator]
    /// hook that substitutes caller-controlled bytes for the
    /// ClientHello rustls would have emitted. The mutator output is
    /// fed into BOTH the wire write and the transcript hash, so the
    /// TLS 1.3 Finished MAC continues to verify.
    ///
    /// `None` (the default) means rustls behaves exactly like the
    /// upstream 0.23.38 release — no behavior change for callers
    /// that don't ask for fingerprint emulation.
    ///
    /// See [`crate::client::mutator`] for the full contract.
    pub client_hello_mutator: Option<Arc<dyn crate::client::ClientHelloMutator>>,
}
