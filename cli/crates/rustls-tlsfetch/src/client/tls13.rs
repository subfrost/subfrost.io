...
fn validate_encrypted_extensions(
    ...
    // tlsfetch fork: when a `ClientHelloMutator` is installed the
    // wire bytes of the CH no longer match `hello.sent_extensions`
    // (which only tracks what rustls itself put in). The mutator
    // routinely adds Chrome-shaped extensions (ALPS, ECH GREASE,
    // application_settings) that the server will legitimately reply
    // to in EncryptedExtensions. Skip the unsolicited check on this
    // path — RFC 8446 §4.2 already requires clients to "ignore
    // unrecognized extensions," so silently dropping them is safe.
    // Without this exemption, Cloudflare-class CDNs alert
    // `UnsolicitedEncryptedExtension` even though the TLS handshake
    // is otherwise complete.
    if config.client_hello_mutator.is_none()
        && hello.server_sent_unsolicited_extensions(exts, &[])
    {
        return Err(common.send_fatal_alert(
            AlertDescription::UnsupportedExtension,
...
impl State<ClientConnectionData> for ExpectEncryptedExtensions {
        ...
        Self: 'm,
    {
        ...
        validate_encrypted_extensions(cx.common, &self.hello, exts, &self.config)?;
        ...
        let ech_retry_configs = match (cx.data.ech_status, &exts.encrypted_client_hello_ack) {
            ...
            //
            // tlsfetch fork: when a `ClientHelloMutator` is installed
            // the wire CH may carry an ECH GREASE extension that
            // rustls's `cx.data.ech_status` doesn't track (the
            // mutator added it after rustls finalized its own
            // state). The server will reply with ECH retry configs
            // in EE because the GREASE wire bytes asked for them.
            // Drop the alert in that case — RFC 9460 §6 explicitly
            // allows GREASE-ECH clients to ignore the retry configs.
            (EchStatus::NotOffered | EchStatus::Accepted, Some(_))
                if self.config.client_hello_mutator.is_none() =>
            {
