...
fn emit_client_hello_for_retry(
    ...
    // tlsfetch fork: ClientHello mutator hook.
    //
    // If the caller installed a `ClientHelloMutator` on the
    // `ClientConfig`, hand them the bytes rustls would have emitted
    // and substitute their replacement into the `encoded` payload
    // BEFORE the transcript-hash add and the wire send. Both
    // operations consume `ch.payload`'s `encoded` field, so swapping
    // it here keeps the local transcript hash in lockstep with what
    // the server hashes off the wire — Finished MAC verification
    // succeeds without any further plumbing.
    //
    // Only the initial ClientHello is mutated. HelloRetryRequest
    // replies (`retryreq.is_some()`) and ECH-inner hellos
    // (`ech_state.is_some()`) are out of scope; see
    // `client/mutator.rs` for the documented limitations.
    if retryreq.is_none() && ech_state.is_none() {
        if let Some(mutator) = config.client_hello_mutator.as_ref() {
            if let MessagePayload::Handshake { encoded, .. } = &mut ch.payload {
                let mutated = mutator.mutate_client_hello(encoded.bytes());
                *encoded = Payload::new(mutated);
            }
        }
    }
