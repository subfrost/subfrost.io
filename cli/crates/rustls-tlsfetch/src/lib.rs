...
pub mod client {
    ...
    pub(super) mod handy;
    mod hs;
    // tlsfetch fork: ClientHello-mutator hook trait. See module docs
    // for the contract and how the transcript hash is kept in sync.
    ...
    // tlsfetch fork: opt-in ClientHello mutator. Re-exported so
    // downstream consumers (tlsfetch-common) can construct one
    // without reaching into private modules.
    pub use mutator::ClientHelloMutator;
