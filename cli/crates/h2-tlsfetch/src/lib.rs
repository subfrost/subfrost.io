...
/// tlsfetch fork: re-export the SETTINGS-order types so
/// downstream consumers (tlsfetch-emulation persona dialer)
/// can configure emission order without enabling the `unstable`
/// feature. Companion type to the `Settings::set_order` API.
pub use crate::frame::SettingKind;
