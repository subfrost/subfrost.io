...
pub struct Settings {
    ...
    /// tlsfetch fork (h2-tlsfetch): optional emission order for
    /// SETTINGS payload. `None` keeps upstream behavior — emit in
    /// the fixed field-declaration order. `Some(seq)` emits the
    /// listed settings in `seq` order, skipping any whose value
    /// is `None`. Used by tlsfetch-emulation's persona dialer to
    /// match a target browser's SETTINGS fingerprint.
    order: Option<Vec<SettingKind>>,
}
...
/// tlsfetch fork (h2-tlsfetch): a value-less companion to
/// [`Setting`] used by [`Settings::set_order`] to pin SETTINGS
/// emission order for persona fingerprint matching.
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub enum SettingKind {
    /// Identifier for `SETTINGS_HEADER_TABLE_SIZE` (RFC 9113 §6.5.2).
    HeaderTableSize,
    /// Identifier for `SETTINGS_ENABLE_PUSH`.
    EnablePush,
    /// Identifier for `SETTINGS_MAX_CONCURRENT_STREAMS`.
    MaxConcurrentStreams,
    /// Identifier for `SETTINGS_INITIAL_WINDOW_SIZE`.
    InitialWindowSize,
    /// Identifier for `SETTINGS_MAX_FRAME_SIZE`.
    MaxFrameSize,
    /// Identifier for `SETTINGS_MAX_HEADER_LIST_SIZE`.
    MaxHeaderListSize,
    /// Identifier for `SETTINGS_ENABLE_CONNECT_PROTOCOL` (RFC 8441).
...
impl Settings {
    ...
    fn for_each<F: FnMut(Setting)>(&self, mut f: F) {
        ...
        // tlsfetch fork: if `order` is set, emit per the
        // operator-supplied sequence (skipping any whose value
        // is `None`). Otherwise fall through to upstream's
        // fixed declaration-order emission.
        if let Some(order) = self.order.as_deref() {
            for kind in order {
                match kind {
                    ...
                    SettingKind::EnableConnectProtocol => {
    ...
    /// tlsfetch fork: set the SETTINGS emission order. Affects
    /// only outbound frames (encode); recv path is unchanged.
    /// `None` restores upstream's fixed declaration-order
    /// emission.
    pub fn set_order(&mut self, order: Option<Vec<SettingKind>>) {
        self.order = order;
    }
}
