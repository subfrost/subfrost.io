/**
 * Unit tests for /api/get-address-unwrap-history endpoint
 */
...
import { POST } from '@/app/api/get-address-unwrap-history/route';
...
function createMockRequest(
  ...
  const url = new URL('http://localhost:3000/api/get-address-unwrap-history');
...
describe('POST /api/get-address-unwrap-history', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('returns unwrap history for a valid address', async () => {
    const mockData = {
      data: {
        items: [
  ...
  it('passes pagination parameters to OYL API', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      'https://mainnet-api.oyl.gg/get-address-unwrap-history',
  ...
  it('returns error response when OYL API fails', async () => {
    ...
    expect(data.error).toBe('Failed to fetch address unwrap history.');
  });
