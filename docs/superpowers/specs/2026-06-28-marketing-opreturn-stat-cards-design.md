...
Janelas: **último dia**, **média móvel 7d** (default), **média 60d**, **período completo**
(primeiro/último/média). Divisões guardam `den===0 → null`.
...
- Query params: `metric`, `template` (`hero|compare`), `window` (`latest|avg7|avg60|full`), `theme`.
